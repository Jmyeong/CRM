...
def l2_loss(pred, target, mask=None):
    """
    pred, target, mask의 L2 Loss를 계산하는 함수
    pred와 target은 (B, 1, H, W) 형태, mask는 (B, 1, H, W) 형태입니다.
    유효한 마스크 값에 대해서만 손실을 계산합니다.
    """
    # 예측값과 실제값의 차이를 구하고 제곱합니다.
    loss = (pred - target) ** 2

    if mask is not None:
        # 마스크가 주어졌다면 유효한 부분만 계산하도록 마스크를 적용합니다.
        # mask 값이 1인 부분만 손실을 계산합니다.
        loss = loss * mask

    # 배치마다 평균 손실을 계산합니다.
    # mask가 주어졌다면, 유효한 픽셀에 대해서만 손실을 계산합니다.
    if mask is not None:
        loss = loss.sum() / (mask.sum() + 1e-6)  # 마스크의 유효한 부분에 대해서만 평균을 계산
    else:
        loss = loss.mean()  # 마스크가 없으면 모든 픽셀에 대해 평균을 계산
...
class Trainer:
    ...
    def run_epoch(self):
        ...
        for batch_idx, inputs in enumerate(self.train_loader):
            ...
            # print(torch.unique(outputs[('disp', 0)]))
    ...
    def process_batch(self, inputs):
        ...
        if self.opt.use_glass_hints:
            glass_mask = inputs["glass_hint_mask"]
            ...
            output_g = inputs['glass_hint']

            for (key, value), scale in zip(outputs.items(), self.opt.scales):
                # if key == ('disp', 0):
                    # print(key, value)
                scale = 3 - scale
                # print(key)
                # print(scale)
                pred_disp = value.clone()
                pred_disp_g = depth_to_disp(output_g.clone(), self.opt.min_depth, self.opt.max_depth)
                # pred_disp_g = normalize_tensor_to_range(pred_disp_g, pred_disp)
                pred_disp_g = torch.nn.functional.interpolate(pred_disp_g, size=(352 // 2 ** scale, 640 // 2 ** scale), mode="bilinear", align_corners=False)
                mask = torch.nn.functional.interpolate(glass_mask.clone(), size=(352 // 2 ** scale, 640 // 2 ** scale), mode="bilinear", align_corners=False)
                pred_disp = torch.nn.functional.interpolate(pred_disp, size=(352 // 2 ** scale, 640 // 2 ** scale), mode="bilinear", align_corners=False)
                # print(pred_disp_g.shape, glass_mask.shape)
                pred_disp_replaced = pred_disp * (mask == 0) + pred_disp_g * (mask > 0)
            ...
            for i, frame_id in enumerate(self.opt.frame_ids[1:]):
                ...
                if self.opt.use_glass_hints:
                    if self.opt.v1_multiscale:
                        ...
                        depth = inputs['glass_hint'] # Output_g를 glass_hint로 하니까 이게 virtual gt로 사용됨 
                        cam_points = self.backproject_depth[source_scale](
                            depth, inputs[("inv_K", source_scale)])
                        pix_coords = self.project_3d[source_scale](
                            cam_points, inputs[("K", source_scale)], T)
    ...
    def compute_loss_masks(reprojection_loss, identity_reprojection_loss,
        ...
        if identity_reprojection_loss is None:
            ...
            if depth_hint_reprojection_loss and torch.all(glass_mask != 0):
            ...
            elif depth_hint_reprojection_loss and not torch.all(glass_mask != 0):
                all_losses = torch.cat([reprojection_loss, depth_hint_reprojection_loss, glass_hint_reprojection_loss], dim=1)
                idxs = torch.argmin(all_losses, dim=1, keepdim=True)
                depth_hint_loss_mask = (idxs == 1).float()
            ...
            if depth_hint_reprojection_loss is not None and torch.all(glass_mask != 0):
                all_losses = torch.cat([reprojection_loss, identity_reprojection_loss,
                                        depth_hint_reprojection_loss], dim=1)
            elif depth_hint_reprojection_loss is not None and not torch.all(glass_mask != 0):
                all_losses = torch.cat([reprojection_loss, identity_reprojection_loss,
    ...
    def compute_losses(self, inputs, outputs):
        ...
        if self.opt.use_glass_hints:
            ...
            inputs['glass_hint_mask'] = F.interpolate(inputs['glass_hint_mask'], (352, 640), mode="bilinear", align_corners=False)
        ...
        for scale in self.opt.scales:
            ...
            if self.opt.use_depth_hints:
                ...
                depth_hint_loss = self.compute_proxy_supervised_loss(pred, target, valid_pixels,
                                                                     depth_hint_loss_mask)
                # print(torch.unique(depth_hint_loss))
                ...
                # loss += reprojection_loss + depth_hint_loss * 0.5 # 0.5는 추가한 것
            ...
            if self.opt.use_glass_hints:
                ...
                glass_hint_loss = self.compute_proxy_supervised_loss(pred, target, valid_pixels,
                                                                     glass_hint_loss_mask)
                # print(torch.unique(glass_hint_loss))
                ...
                # loss += reprojection_loss + depth_hint_loss * 0.5 + glass_hint_loss # 0.5는 추가한 것
                # print(glass_hint_loss)
                # print(pred.shape)
                # print(target.shape)
                # print(inputs['mask'].shape)
                # print()
                glass_loss = 0.0
                if len(torch.unique(inputs['mask'])) > 1:  # 마스크가 0과 1 이상 포함되는 경우
                    valid_mask = (inputs['mask'] > 0).float()  # 마스크가 0보다 큰 부분만 사용
                    glass_loss = l2_loss(pred * valid_mask, target * valid_mask)
                    losses['l2_loss/{}'.format(scale)] = glass_loss
                # print(glass_loss)
            loss += reprojection_loss + depth_hint_loss * 0.5 + glass_hint_loss + glass_loss * 1.5
            ###
        ...
        for j in range(min(4, self.opt.batch_size)):  # write a maxmimum of four images
            for s in self.opt.scales:
                ...
                if self.opt.use_glass_hints:
                    ...
                    if s == 0:
                        disp = 1 / (inputs['glass_hint'] + 1e-7) # glass_hint_mask
                        writer.add_image(
                            "glass_hint/{}".format(j),
                            normalize_image(disp[j]), self.step)
                        
                        writer.add_image(
                            "glass_hint_mask{}/{}".format(s, j),
                            inputs["glass_hint_mask"][j], self.step)
                        ...
                        # writer.add_image(
                        #     "features_Test{}".format(j),
                        #     self.features[j], self.step)
    def save_opts(self):
