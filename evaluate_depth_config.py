...
def evaluate(opt):
    ...
    if opt.ext_disp_to_eval is None:
        ...
        input = torch.randn(1, 3, 352, 640).to("cuda")
        input_size = (3, 352, 640)

        flops = FlopCountAnalysis(model, input)
        print(f"Total FLOPs: {flops.total() / 1e9:.2f} GFLOPs")
        with torch.cuda.device(0):  # CUDA 디바이스 사용 시
        ...
        with torch.no_grad():
            for data in dataloader:
                ...
                # print(f"input_color shape: {input_color.shape}")
                # print(opt.batch_size)
                print(encoder(input_color).shape)
                output = depth_decoder(encoder(input_color))
                if opt.log_attn:
                    # attn = output[("attn", 0)]
                    writer = writers["vis"]
                    for j in range(min(4, opt.batch_size, input_color.size(0))):  # write a maxmimum of four images
                        writer.add_image(
                            "color_{}/{}".format(0, j),
                ...
                pred_disp_resized = cv2.resize(pred_disp[0], (640, 360))
                # print(pred_disp_resized.shape)
                # print(np.unique(pred_disp_resized))
                pred_disps_save.append(pred_disp_resized)
    ...
    if opt.no_eval:
        ...
        for idx in range(len(pred_disps)):
            disp_resized = cv2.resize(pred_disps[idx], (640, 360))
