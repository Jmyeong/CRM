...
class Trainer:
    ...
    def predict_poses(self, inputs, features):
        ...
        if self.num_pose_frames == 2:
            ...
            # print(torch.unique(inputs["color_aug", 0, 0]))
            if self.opt.pose_model_type == "shared":
            ...
            for f_i in self.opt.frame_ids[1:]:
                if f_i != "s":
                    ...
                    # print(f"extrinsic 0 -> {f_i}:", outputs[("cam_T_cam", 0, f_i)])
    ...
    def generate_images_pred(self, inputs, outputs):
        ...
        for scale in self.opt.scales:
            ...
            #     mask = inputs["mask"]
                ...
                # print(padded_disp.shape, replace_pixel.shape)
                # outputs["padded_disp", scale] = padded_disp
                # outputs["replace_pixels", scale] = replace_pixel
            ...
            if self.opt.use_glass_hints:
                ...
                processed = []
                # print(depth.shape)
                for bn in range(depth.size(0)):
                    start = time.time()
                    depth_refined = self.crm(depth[bn], inputs["mask", scale][bn])
                    if len(depth_refined.shape) < 3:
                        depth_refined = depth_refined.unsqueeze(0)
                    # print(depth_refined.shape)
                    # print(torch.unique(inputs["mask", scale][bn]))
                    processed.append(depth_refined)
                    end = time.time()
                    # print(f"processing time : {end - start:.4f}s")
                processed = torch.stack(processed)
                # print(processed.shape)
                outputs["refined_depth", scale] = processed
                ## Save pseudo depth gt
                if scale == 0:
                    for batch in range(processed.shape[0]):
                        path = inputs["path"][batch]
                        depth_batch = processed[batch]
                        # print(path)
                        date_drive = path.split(" ")[0]
                        index = path.split(" ")[1]
                        side = path.split(" ")[-1]
                        if side == "r":
                            image_num = 3
                            ...
                            image_num = 2
                        output_folder = os.path.join("/ssd1/jm_data/depth/ssl/monodepth2/jbnu_stereo", date_drive, "proj_depth", "groundtruth",  f"image_0{image_num}")
                        os.makedirs(output_folder, exist_ok=True)
                        output_path = os.path.join(output_folder, f"{str(index).zfill(10)}.npy")
                        refined_depth = depth_batch.detach().squeeze(0).cpu().numpy()
                        np.save(output_path, refined_depth)
                        # print(output_path)
    ...
    def compute_losses(self, inputs, outputs):
        ...
        for scale in self.opt.scales:
            ...
            outputs["reprojection_loss_mask/{}".format(scale)] = reprojection_loss_mask
            ...
            # pred_depth = F.interpolate(depth_, [360, 640], mode="bilinear", align_corners=False)
            # gt_depth = inputs["depth_gt"]
            # mask = (gt_depth > 0).float()

            # scale_loss = torch.abs(pred_depth - gt_depth)
            # scale_loss = (scale_loss * mask).sum() / (mask.sum() + 1e-6)
            # print("requires_grad:", pred_depth.requires_grad)
            # print("scale_loss grad_fn:", scale_loss.grad_fn)  # None이면 역전파 안 됨
            ...
            # silogloss = 0.0
            # if self.opt.use_glass_hints:
            #     silogloss = silog_loss(outputs["padded_disp", scale], outputs["disp", scale], inputs["mask", scale])
            #     # print(silogloss.item())
            #     if str(silogloss.item()) == "nan":
            #         # print(1)
            #         silogloss = 0.0
            #     losses["silogloss/{}".format(scale)] = silogloss
            #     # print(l2_loss)
            #     # print(type(self.mask), type(outputs["padded_disp", scale]), type(outputs["disp", scale]))
            loss += reprojection_loss + depth_hint_loss * 0.5
    ...
    def compute_depth_losses(self, inputs, outputs, losses):
        ...
        print(f"before median scaling : {torch.unique(depth_pred)}")
    ...
    def log(self, mode, inputs, outputs, losses):
        ...
        for j in range(min(4, self.opt.batch_size)):  # write a maxmimum of four images
            for s in self.opt.scales:
                ...
                writer.add_image(
                    "reprojection_loss_mask_{}/{}".format(s, j),
                    outputs[f"reprojection_loss_mask/{s}"][j], self.step)
                if self.opt.predictive_mask:
                ...
                if self.opt.use_glass_hints:
                    for s in self.opt.scales:
                        writer.add_image(
                            "refined_depth_{}/{}".format(s, j),
                            normalize_image(outputs[("refined_depth", s)][j]), self.step)
                        # writer.add_image(
                        #     "replace_pixels_{}/{}".format(s, j),
                        #     normalize_image(outputs[("replace_pixels", s)][j]), self.step)
                    # writer.add_image(
                    #     "decoder",
                    #     normalize_image(self.decoder_feature[0][0].unsqueeze(0)), self.step)
    def save_opts(self):
