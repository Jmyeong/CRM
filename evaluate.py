...
def depth_to_colormap_array(depth_map, cmap='Spectral'):
    """
    단일 채널 깊이맵을 RGB 이미지(numpy array)로 변환
    depth_map: numpy 2D array
    cmap: matplotlib colormap
    return: H x W x 3 RGB 이미지 (uint8)
    """
    norm = plt.Normalize(vmin=np.min(depth_map), vmax=np.max(depth_map))
    colormap = plt.get_cmap(cmap)
    depth_color = colormap(norm(depth_map))[:, :, :3]  # RGBA → RGB
    return depth_color

def normalize_to_255(image):
    """
    단일 채널 이미지를 0~255 범위로 정규화 (uint8 변환)
    """
    img_min, img_max = image.min(), image.max()
    if img_max - img_min < 1e-8:
        return np.zeros_like(image, dtype=np.uint8)
    
    norm_img = (image - img_min) / (img_max - img_min)  # 0~1
    norm_img = (norm_img * 255).astype(np.uint8)        # 0~255
    return norm_img
...
class RunEvaluate():
    ...
    def evaluate(self):
        ...
        if self.mode == "total":
            filenames = readlines(os.path.join(self.splits_dir, self.opt.split, "test_files.txt"))
        elif self.mode == "transp":
            filenames = readlines(os.path.join(self.splits_dir, self.opt.split, "transp_files.txt"))
        else:
        ...
        ratios = []
        errors = []
        out_dir = os.path.join(project_root, "sqldepth_crm", "pred_results", self.mode, "ours")
        os.makedirs(out_dir, exist_ok=True)
        os.makedirs(os.path.join(out_dir, "colormap"), exist_ok=True)
        ...
        with torch.no_grad():
            for i, data in enumerate(tqdm(dataloader)):
                ...
                pred_vis = depth_to_colormap_array(pred_depth)
                pred_vis = normalize_to_255(pred_vis)
                
                save_img(pred_depth, os.path.join(out_dir, f"{str(i).zfill(10)}.png"))
                # np.save(os.path.join(out_dir, f"{str(i).zfill(10)}.npy"), pred_depth)
                img_vis = F.interpolate(input_color, [pred_depth.shape[-2], pred_depth.shape[-1]], mode="bilinear", align_corners=False)
                img_vis = squeezing(to_numpy(img_vis))
                img_vis = normalize_to_255(img_vis)
                
                concat_img = np.concatenate([img_vis, pred_vis], axis=1)
                concat_img = cv2.cvtColor(concat_img, cv2.COLOR_BGR2RGB)
                # vis_imgs(concat_img=concat_img, pred=pred_depth)
                # cv2.imwrite(os.path.join(out_dir, f"colormap/{str(i).zfill(10)}.png"), concat_img)
                ...
                transp_mask = cv2.resize(transp_mask, (mask.shape[1], mask.shape[0]))
                
                if self.is_mask:
                    if not self.in_mask:
                        out_mask = 255 - transp_mask
                        # print(mask.shape, transp_mask.shape)
                ...
                if not len(pred_depth) == 0 or not len(gt_depth) == 0:
                    print(len(pred_depth), len(gt_depth))

                    pred_depth *= self.STEREO_SCALE_FACTOR
