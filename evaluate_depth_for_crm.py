...
def evaluate(opt):
        ...
        with torch.no_grad():
            for data in dataloader:
                ...
                if is_crm:
                    ...
                    processed = []
                    for bn in range(disp.size(0)):
                        pseudo = disp[bn]
                        # print(disp.shape, mask.shape)
                        mask_bn = mask[bn]
                        pseudo = F.interpolate(pseudo.unsqueeze(0), (mask.shape[-2], mask.shape[-1]), mode="bilinear", align_corners=False)
                        pseudo_refined = crm(pseudo.squeeze(0), mask_bn)
    ...
    if opt.save_pred_disps:
        output_path = os.path.join(
            opt.load_weights_folder, "disps_{}_split_crm.npy".format(opt.eval_split))
