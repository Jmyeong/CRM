...
class RunEvaluate():
    def __init__(self, mode="total", is_mask=True, in_mask=True, opt=None):
        ...
        self.crm = CRM(self.K).cuda()
        self.K = to_tensor_(self.K)
        self.teacher_path = "./logs/revision/teacher/models/weights_24"
    ...
    def evaluate(self):
        ...
        print("-> Loading weights from {}".format(self.teacher_path))
        if self.mode == "total":
        ...
        encoder_path = os.path.join(self.teacher_path, "encoder.pth")
        decoder_path = os.path.join(self.teacher_path, "depth.pth")
        ...
        with torch.no_grad():
            for i, data in enumerate(tqdm(dataloader)):
                ...
                t_mask = F.interpolate(transp_mask, (360, 640), mode="bilinear", align_corners=False)
                t_mask = squeezing(squeezing(t_mask)).cuda()
                out_crm, _ = self.crm(squeezing(squeezing(pred_depth)), t_mask)
...
if __name__ == "__main__":
    ...
    run_eval = RunEvaluate(mode="total", is_mask=True, in_mask=True, opt=opt)
    run_eval.evaluate()
