...
def load_pretrained_model(model, checkpoint_path="/ssd1/jm_data/depth/ssl/monodepth2/logs/jbnu_stereo_scratch/models/weights_19/", device=None):
...
class Trainer:
    ...
    def process_batch(self, inputs):
        ...
        if self.opt.pose_model_type == "shared":
            ...
            # print(outputs)
            outputs_g = None
            # print(torch.unique(inputs[("color_glass_aug", 0, 0)]))
        ...
        if outputs_g is not None:
            ...
            masked_uniform_loss = masked_uniformity_loss(outputs, inputs["glass_mask"])
            # print(silogloss)
            # print(l2Loss * 0.5 + l1Loss * 0.5 + silogloss)
            # print(masked_uniform_loss)
            # print()
            losses['loss'] = losses['loss'] + l2Loss * 0.5 + l1Loss * 0.5 + silogloss * 0.5 + masked_uniform_loss * 0.5
    ...
    def compute_loss_masks(reprojection_loss, identity_reprojection_loss,
                           depth_hint_reprojection_loss, glass_hint_reprojection_loss):
        ...
        if identity_reprojection_loss is None:
            ...
            if depth_hint_reprojection_loss is not None and glass_hint_reprojection_loss is not None:
    ...
    def compute_losses(self, inputs, outputs):
        ...
        if self.opt.use_depth_hints and self.opt.use_glass_hints:
            ...
            glass_hint_reproj_loss += 1000 * (1 - inputs['glass_hint_mask'])
        ...
        for scale in self.opt.scales:
            ...
            if self.opt.use_glass_hints:
                ...
                target = self.outputs_g[('depth_g', 0, scale)]
                pred = outputs[('depth', 0, scale)]
            ...
            ###
            loss += reprojection_loss + depth_hint_loss + glass_hint_loss * 0.5 # 0.5는 추가한 것
    ...
    def log(self, mode, inputs, outputs, losses):
        ...
        for j in range(min(4, self.opt.batch_size)):  # write a maxmimum of four images
            for s in self.opt.scales:
                ...
                if self.opt.use_glass_hints:
                    if s == 0:
                        disp = 1 / (self.outputs_g[('depth_g', 0, s)] + 1e-7) * inputs['glass_hint_mask']
                        writer.add_image(
                            "glass_hint/{}".format(j),
                            normalize_image(disp[j]), self.step)
