...
class Trainer:
    ...
    def process_batch(self, inputs):
        ...
        total_loss = 0
        gt_depth = inputs["depth_gt"]              # (B, 1, 360, 640)
        mask = gt_depth > 0                        # valid GT mask

        for scale in range(self.num_scales):
            disp = outputs[("disp", scale)]        # (B, 1, Hs, Ws)
            _, pred_depth = disp_to_depth(disp, self.opt.min_depth, self.opt.max_depth)

            # 📌 예측 depth를 GT 해상도 (360, 640)로 upsample
            pred_depth_up = F.interpolate(pred_depth, size=gt_depth.shape[-2:], mode='bilinear', align_corners=False)
            outputs[('depth', 0, scale)] = pred_depth_up
            valid_mask = mask & (pred_depth_up > 0)

            # 📌 loss는 업샘플된 pred vs 원본 GT 기준으로 계산
            loss = self.silogloss(pred_depth_up[valid_mask], gt_depth[valid_mask])
            total_loss += loss
