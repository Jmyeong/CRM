...
from padding import padding_disp, padding_disp_batch
from tqdm import tqdm

from feature_fusion import FeatureFusion, Fusion, PatchExtractor
# from dino.dino import dino
# from dino.conv import Conv
...
class PatchEmbeddingNet(nn.Module):
    ...
    def forward(self, x, attn=None):
        ...
        # print(feat_x.shape)
        return self.project(feat_x)
    
def sample_patches_from_mask_batch(images, masks, size=32, is_positive=True, max_tries=100):
    """
    이미지 배치에서 각 이미지마다 조건에 맞는 patch 추출
    
    Args:
        images: (B, C, H, W) - 이미지 텐서
        masks:  (B, H, W) - 바이너리 마스크 텐서 (0 or 1)
        size: 추출할 patch 크기
        is_positive: True면 mask 영역에서, False면 배경에서 추출
        max_tries: patch 못 찾을 때를 위한 최대 반복 횟수
    
    Returns:
        patches: (B, C, size, size)
    """
    ...
    for b in range(B):
        ...
        for _ in range(max_tries):
            ...
            region = mask[top:top+size, left:left+size].float()
            # print(region)
            mean_val = region.float().mean().item()
            # print(mean_val)
            if is_positive and mean_val > 0.8:
                # print("positive")
                patches.append(patch)
                found = True
                break
            elif not is_positive and mean_val < 0.1:
                # print("negative")
                patches.append(patch)
                found = True
                break
        ...
        if not found:
            # fallback: 그냥 중앙에서 crop
            # print("not found")
            patch = img[:, H//2:H//2+size, W//2:W//2+size]
            patches.append(patch)

    return torch.stack(patches)  # (B, C, size, size)

def weighted_triplet_loss(anchor, positive, negative, pos_weight=1.0, margin=1.0):
    d_ap = F.pairwise_distance(anchor, positive, p=2)
    d_an = F.pairwise_distance(anchor, negative, p=2)
    
    loss = F.relu(pos_weight * d_ap - d_an + margin)
    return loss  # shape: (B,)

def silog_loss(pred: torch.Tensor, target: torch.Tensor, mask: torch.Tensor, lam=0.85, eps=1e-6):
    """
    pred, target: (B, 1, H, W) or (B, H, W)
    mask: (B, 1, H, W) or (B, H, W), binary float
    lam: weight for scale-invariant term
    """
    ...
    log_diff2 = log_diff ** 2

    silog = log_diff2.mean() - lam * (log_diff.mean() ** 2)
    return silog

def inpaint_color_tensor(color: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
    """
    color: (B, 3, H, W) or (3, H, W) - float32 타입 (예: 0~1 또는 0~255 정규화된 이미지)
    mask:  (B, 1, H, W) or (H, W) - binary mask (0 또는 1)
    """
    if color.dim() == 3:
        _, H, W = color.shape
        fill_color = torch.randint(0, 256, (3,), device=color.device).to(dtype=color.dtype)
        masked = color.clone()
        idx = mask.nonzero(as_tuple=True)
        if len(idx[0]) > 0:
            masked[:, idx[0], idx[1]] = fill_color.view(3, 1)
        return masked
        ...
        for i in range(B):
            fill_color = torch.randint(0, 256, (3,), device=color.device).to(dtype=color.dtype)
            mask_i = mask[i, 0] > 0
            idx = mask_i.nonzero(as_tuple=True)
            if len(idx[0]) > 0:
                masked[i][:, idx[0], idx[1]] = fill_color.view(3, 1)
        return masked

    else:
        raise ValueError("color must be of shape (3, H, W) or (B, 3, H, W)")
...
class Trainer:
    def __init__(self, options):
        ...
        def hook_fn(module, input, output):
            # print("[hook] feature shape:", output.shape)
            hook["decoder_feature"] = output
            self.decoder_feature = output
        ...
        target_layer = self.models["depth"].decoder[6]
        hook_handle = target_layer.register_forward_hook(hook_fn)
        self.feature_extract = FeatureFusion(3, 1).to(self.device)
        self.fusion = Fusion().to(self.device)
        self.patchembed = PatchExtractor().to(self.device)
    ...
    def process_batch(self, inputs):
        ...
        if self.opt.pose_model_type == "shared":
            ...
            # dino_features = dino(inputs["color_aug", 0, 0])
            # conv_features = self.conv(dino_features)
            # features[-1] = features[-1] + conv_features
            # self.features = features[-1][:, 0, :, :].detach().clone().unsqueeze(1)
    ...
    def generate_images_pred(self, inputs, outputs):
        ...
        for scale in self.opt.scales:
            ...
            if self.opt.use_glass_hints:
                ...
                # print(mask.shape)
                # print(disp.shape)
                inputs["mask", scale] = F.interpolate(mask, [disp.shape[2], disp.shape[3]], mode='bilinear', align_corners=False)
                padded_disp = padding_disp_batch(disp, inputs["mask", scale]).squeeze(1)
                
                # rgb_patch = self.patchembed(inputs[('color', 0, scale)])
                # disp_patch = self.patchembed(padded_disp)
                # print(f"inputs : {inputs[('color', 0, scale)].shape}, disp : {disp.shape}")
                fused_features = self.feature_extract(inputs[('color', 0, scale)], disp)[0]
                outputs["encoded", scale] = self.feature_extract(inputs[('color', 0, scale)], disp)[1]
                # final_disp = self.fusion(fused_features)
                # final_disp = self.models["depth"](fused_features)
                
                outputs["padded_disp", scale] = padded_disp
                outputs["feature_disp", scale] = fused_features
                # print(padded_disp.shape)
                # disp = padded_disp.squeeze(1)
                # print(disp.shape)
            ...
            for i, frame_id in enumerate(self.opt.frame_ids[1:]):
                ...
                # if self.opt.use_glass_hints:
                #     outputs[("color_padding", frame_id, scale)] = F.grid_sample(
                #         inputs[("color", frame_id, source_scale)],
                #         outputs[("sample", frame_id, scale)],
                #         padding_mode="border")
    ...
    def compute_losses(self, inputs, outputs):
        ...
        for scale in self.opt.scales:
            ...
            # mask = inputs["mask"]
            # target = inpaint_color_tensor(target, mask)
            
            for frame_id in self.opt.frame_ids[1:]:
                pred = outputs[("color", frame_id, scale)]
                # mask = inputs["mask"]
                # reproj_loss = self.compute_reprojection_loss(pred, target)
                # print(reproj_loss.shape)
                # masked_loss = reproj_loss * (1 - mask)
                # loss = masked_loss.mean()  # 🔹 scalar로 변환
                # print(masked_loss.shape)
                reprojection_losses.append(self.compute_reprojection_loss(pred, target))
                # reprojection_losses.append(masked_loss)
            ...
            ## 여기에 padding 추가
            ## padded dispaity와 L2 loss 계산
            silogloss = 0.0
            if self.opt.use_glass_hints:
                silogloss = silog_loss(outputs["padded_disp", scale], outputs["feature_disp", scale], inputs["mask", scale])
                losses["silogloss/{}".format(scale)] = silogloss
                # print(l2_loss)
                # print(type(self.mask), type(outputs["padded_disp", scale]), type(outputs["disp", scale]))
            loss += reprojection_loss + depth_hint_loss * 0.5 + silogloss  # 0.5는 추가한 것
        ...
        for j in range(min(4, self.opt.batch_size)):  # write a maxmimum of four images
            for s in self.opt.scales:
                ...
                if self.opt.use_glass_hints:
                    for s in self.opt.scales:
                        writer.add_image(
                            "padded_disp_{}/{}".format(s, j),
                            normalize_image(outputs[("padded_disp", s)][j]), self.step)
                        writer.add_image(
                            "feature_disp_{}/{}".format(s, j),
                            normalize_image(outputs[("feature_disp", s)][j]), self.step)
    def save_opts(self):
